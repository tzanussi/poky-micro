...
class MicroMinnowmaxPlugin(SourcePlugin):
    name = 'micro-minnowmax-64'
    ...
    def do_configure_grubefi(self, hdddir, cr, cr_workdir):
        ...
        grubefi_conf += "linux %s root=/dev/ram0 rootwait %s\n" \
            % (kernel, options)

        machine = "minnowmax-64"
        grubefi_conf += "initrd /image-micro-%s.cpio.gz\n" % (machine)

        grubefi_conf += "}\n"

        grubefi_conf += "menuentry 'boot to persistent rootfs'{\n"
    ...
    def do_prepare_partition(self, part, source_params, cr, cr_workdir,
        ...
        try:
            if source_params['loader'] == 'grub-efi':
                ...
                cp_cmd = "cp %s/*.efi %s/EFI/BOOT" % (bootimg_dir, hdddir)
